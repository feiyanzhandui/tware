...
class Q6Worker3 : public Worker {
public:
    Q6Worker3(int id, int numWorkers, ZmqSocket *in, ZmqSocket *out,
            vector<string> files)
    ...
    virtual void init() {
        ...
        l_shipdate = new short[Q6_LINEITEM];
    ...
    virtual void execute() {
        ...
        short *t_shipdate = l_shipdate;
        float bitmap[TILE];
        float revenue = 0;
        for (int i = 0; i < Q6_LINEITEM / TILE; i++) {
            for (int j = 0; j < TILE; j++)
                bitmap[j] = (float) (t_shipdate[j] >= Q6_SDATE1
                          & t_shipdate[j] < Q6_SDATE2
                          & t_discount[j] >= 0.05f
                          & t_discount[j] <= 0.07f
                          & t_quantity[j] < 24.0f);
            for (int j = 0; j < TILE; j++)
        ...
        for (int i = Q6_LINEITEM - Q6_EXTRA; i < Q6_LINEITEM; i++)
            if (l_shipdate[i] >= Q6_SDATE1
             && l_shipdate[i] < Q6_SDATE2
             && l_discount[i] >= 0.05f
             && l_discount[i] <= 0.07f
             && l_quantity[i] < 24.0f)
