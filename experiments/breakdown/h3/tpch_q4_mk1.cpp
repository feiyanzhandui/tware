...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < Q4_ORDERS; i += TILE) {
        for (int j = 0; j < TILE; j++)
            bitmap[j] = (o_orderdate[i + j] >= Q4_DATE1)
                      * (o_orderdate[i + j] < Q4_DATE2);
        for (int j = 0; j < TILE; j++) {
            int key = o_orderkey[i + j] % KEYS + 1;
            for (int k = 0; k < exists[key].size(); k++)
                if (o_orderkey[i + j] == exists[key][k]) {
                    vals[o_orderpriority[i + j]] += bitmap[j];
                    break;
                }
        }
    }
