...
__attribute__((noinline)) bool pred1(long l_commitdate, long l_receiptdate) {
    return l_commitdate < l_receiptdate;
}

__attribute__((noinline)) void addExists(unordered_set<int> *exists,
        int l_orderkey) {
    exists->insert(l_orderkey);
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < Q4_LINEITEM; i++)
        if (pred1(l_commitdate[i], l_receiptdate[i]))
            addExists(exists, l_orderkey[i]);
