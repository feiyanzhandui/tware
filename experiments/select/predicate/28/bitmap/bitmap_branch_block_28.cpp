...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < TILES; i++) {
        ...
        for (int j = 0; j < TILE; j++)
            if (bitmap[j]) {
                res1.back()[pos] = tile1[i];
                res2.back()[pos] = tile2[i];
                res3.back()[pos] = tile3[i];
                res4.back()[pos] = tile4[i];
                res5.back()[pos] = tile5[i];
                res6.back()[pos] = tile6[i];
                res7.back()[pos] = tile7[i];
                res8.back()[pos++] = tile8[i];
                if (pos == TILE) {
                    res1.push_back(new int[TILE]);
                    res2.push_back(new int[TILE]);
                    res3.push_back(new int[TILE]);
